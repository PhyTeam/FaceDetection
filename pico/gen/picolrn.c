...
static float stop_condition = 1e-7f;
...
int learn_with_default_parameters(char* trdata, char* dst)
{
	...
	//
	while(sample_training_data(tvals, rs, cs, ss, iinds, os, &np, &nn) > stop_condition)
	{
...
int main(int argc, char* argv[])
{
	...
	{
		if (argc == 4) {
				sscanf(argv[3], "%f", &stop_condition); // load stop condition default 1e-6f;
		}
