...
int run_cascade(void *cascade, float *o, int r, int c, int s, void *vppixels, int nrows, int ncols,
                int ldim) {
    //
    int i, j, idx;
    ...
    if ((r + 128 * s) / 256 >= nrows || (r - 128 * s) / 256 < 0 || (c + 128 * s) / 256 >= ncols ||
        (c - 128 * s) / 256 < 0)
        return -1;

    //
    offset = ((1 << tdepth) - 1) * sizeof(int32_t) + (1 << tdepth) * sizeof(float) +
             1 * sizeof(float);
    ...
    *o = 0.0f;
    ...
    for (i = 0; i < ntrees; ++i) {
        ...
        thr = *(float *) (ptree + ((1 << tdepth) - 1) * sizeof(int32_t) +
                          (1 << tdepth) * sizeof(float));

        //
        idx = 1;

        for (j = 0; j < tdepth; ++j)
            idx = 2 * idx + (pixels[(r + tcodes[4 * idx + 0] * s) / 256 * ldim +
                                    (c + tcodes[4 * idx + 1] * s) / 256] <=
                             pixels[(r + tcodes[4 * idx + 2] * s) / 256 * ldim +
                                    (c + tcodes[4 * idx + 3] * s) / 256]);
...
int run_rotated_cascade(void *cascade, float *o, int r, int c, int s, float a, void *vppixels,
    ...
    static int qcostable[
            32 + 1] = {256, 251, 236, 212, 181, 142, 97, 49, 0, -49, -97, -142, -181, -212, -236,
                       -251, -256, -251, -236, -212, -181, -142, -97, -49, 0, 49, 97, 142, 181, 212,
                       236, 251, 256};
    static int qsintable[
            32 + 1] = {0, 49, 97, 142, 181, 212, 236, 251, 256, 251, 236, 212, 181, 142, 97, 49, 0,
                       -49, -97, -142, -181, -212, -236, -251, -256, -251, -236, -212, -181, -142,
                       -97, -49, 0};
    ...
    if ((r + 46341 * s) / 65536 >= nrows || (r - 46341 * s) / 65536 < 0 ||
        (c + 46341 * s) / 65536 >= ncols || (c - 46341 * s) / 65536 < 0)
        return -1;
...
#pragma omp parallel
        {
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "picornt: ", "Number of threads is %d", num_threads);
        }

#ifdef _OPENMP
        __android_log_print(ANDROID_LOG_DEBUG, "picornt: ", "OpenMP running");
#endif
#pragma omp parallel for shared(ndetections)*/
        for (r = s / 2 + 1; r <= nrows - s / 2 - 1; r += dr)
            for (c = s / 2 + 1; c <= ncols - s / 2 - 1; c += dc) {
                ...
                if (0.0f == angle)
                    ...
                    t = run_rotated_cascade(cascade, &q, r, c, s, angle, pixels, nrows, ncols,
                                            ldim);
...
void ccdfs(int a[], int i, float rcsq[], int n) {
    ...
    for (j = 0; j < n; ++j)
        if (a[j] == 0 &&
            get_overlap(rcsq[4 * i + 0], rcsq[4 * i + 1], rcsq[4 * i + 2], rcsq[4 * j + 0],
                        rcsq[4 * j + 1], rcsq[4 * j + 2]) > 0.3f) {
            //
            a[j] = a[i];
